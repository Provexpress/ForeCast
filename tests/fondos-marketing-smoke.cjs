...
assert.match(html, /src\/styles\/main\.css\?v=20260818-fondos-rio2/);
...
assert.match(html, /src\/scripts\/fondos-marketing\.js\?v=20260818-fondos-rio2/);
assert.match(html, /src\/scripts\/main\.js\?v=20260818-fondos-rio2/);
...
assert.match(fondos, /cop: 7000000, obs: 'Factura pendiente'/);
assert.match(fondos, /id: 'pop'/);
assert.match(fondos, /getRowsWithValueAndLabel/);
assert.match(fondos, /findRowByLabel\(totalSheet, 'A', 'Total'\)/);
...
assert.doesNotMatch(fondos, /class="kpi-card kpi-commission"/);
assert.doesNotMatch(fondos, /comisi[oó]n/i);
assert.match(fondos, /id="fondos-rio-funding"/);
assert.match(fondos, /function renderRioFunding\(\)/);
assert.match(fondos, /Recaudo Río · 20 al 24 de mayo/);
assert.match(fondos, /Valor por cupo/);
assert.match(fondos, /XLSX\.utils\.book_append_sheet\(wb, wsRio, 'Recaudo Río'\)/);
assert.match(fondos, /'Saldo Cisco'/);
